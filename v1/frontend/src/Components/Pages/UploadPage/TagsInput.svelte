...
<div class="md:col-span-2">
  <label for="tags" class="block text-sm font-medium text-[#1F2937] mb-2"> Tags </label>
  <div id="tags" class="flex flex-wrap gap-2 mb-3">
    {#each $formData.tags as tag}
      <span
        class="inline-flex items-center px-3 py-1 bg-[#8B5CF6]/10 text-[#8B5CF6] rounded-full text-sm"
      >
        {tag}
        <button type="button" onclick={() => removeTag(tag)} class="ml-2 hover:text-[#8B5CF6]/70">
          <X class="h-3 w-3" />
        </button>
      </span>
    {/each}
