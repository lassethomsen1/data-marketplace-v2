<script>
  ...
  function handleFiles(files) {
    ...
    const filesWithIds = validFiles.map(file => Object.assign(file, { id: crypto.randomUUID() }));
...
<div class="bg-[#F3F4F6] rounded-xl p-6">
  ...
  <div
    class="border-2 border-dashed rounded-lg p-8 text-center transition-all duration-200 {isDragOver
      ? 'border-[#3B82F6] bg-[#3B82F6]/5'
      : 'border-[#6B7280]/30 hover:border-[#3B82F6]/50'}"
  ...
  >
    ...
    <label
      class="inline-flex items-center px-4 py-2 bg-[#3B82F6] text-white rounded-lg hover:bg-[#2563EB] transition-colors cursor-pointer"
    >
    ...
    <p class="text-sm text-[#6B7280] mt-2">Supports CSV and JSON files up to 100MB eah</p>
  </div>
  ...
  {#if $uploadedFiles.length > 0}
    <div class="mt-6 space-y-3">
      ...
      {#each $uploadedFiles as file (file.id)}
        <div
          class="flex items-center justify-between bg-white rounded-lg p-3 border border-[#E5E7EB]"
        >
