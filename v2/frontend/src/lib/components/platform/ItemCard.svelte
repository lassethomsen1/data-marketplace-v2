...
<div class="flex flex-col border border-blue-100 rounded-lg p-4 hover:scale-[1.01] hover:shadow-lg">
  ...
  <p class="text-sm text-[#6B7280] mb-4">
    By <span class="text-[#0EA5E9] font-medium">{item.author}</span>
  </p>
  ...
  <button
    onclick={() => goto(`/dataset/${item.id}`)}
    class="bg-[#3B82F6] hover:bg-[#1E40AF] text-white text-sm py-2 px-4 rounded-md font-medium"
  >
    Buy Now
  </button>
</div>
